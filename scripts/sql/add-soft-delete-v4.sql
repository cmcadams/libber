-- ── add-soft-delete-v4.sql ────────────────────────────────────────────────────
--
-- Soft delete for stores + customer membership removal.
-- Supersedes add-soft-delete-v3.sql.
--
-- Changes from v3:
--   - Sections reorganised by caller role (admin / customer / staff-admin).
--   - Inline step comments standardised in every mutation function body.
--   - NULL guards added to award_points, adjust_points, admin_create_outlet.
--   - Error message casing standardised throughout ('not authorized').
--   - Above-function check-order lists removed (inline comments are the record).
--   - Grants grouped to match section order.
--
-- No signature changes. No business logic changes.
...
-- apply_for_staff: submits a staff application. Archived stores are rejected.
...
-- approve_staff_applicant: promotes an applicant to staff.
